...
def high_result(dict_result: dict) -> list:
    """
    ...
    if dict_result["sort"] == "PRICE_LOW_TO_HIGH":
        for count_hotel in range(int(dict_result["count"])):
            hotel_id = response_2["data"]["propertySearch"]["properties"][count_hotel]["id"]
            ...
            info_hotels_list.append(
                {
                    ...
                    "sort_element": f"Цена за одни сутки: {round(response_2['data']['propertySearch']['properties'][count_hotel]['price']['lead']['amount'], 1)} USD",
        ...
        for count_hotel in range(int(dict_result["count"])):
            ...
            info_hotels_list.append(
                {
                    "name_hotels": f"Название отеля: {response_3['data']['propertyInfo']['summary']['name']}",
                    "sort_element": f"Оценка: {float(response_2['data']['propertySearch']['properties'][count_hotel]['reviews']['score'])}",
                    "url": f"Ссылка отеля на сайте Hotels: https://www.hotels.com/h{hotel_id}.Hotel-Information"
                }
            )
            count_photo = 3
        ...
        for count_hotel in range(int(dict_result["count"])):
            ...
            info_hotels_list.append(
                {
                    "name_hotels": f"Название отеля: {response_3['data']['propertyInfo']['summary']['name']}",
                    "sort_element": f"Расстояние до центра: {response_2['data']['propertySearch']['properties'][count_hotel]['destinationInfo']['distanceFromDestination']['value']} км.",
                    "url": f"Ссылка отеля на сайте Hotels: https://www.hotels.com/h{hotel_id}.Hotel-Information"
                }
            )
            count_photo = 3
        ...
        for count_hotel in range(int(dict_result["count"])):
            ...
            info_hotels_list.append(
                {
                    "name_hotels": f"Название отеля: {response_3['data']['propertyInfo']['summary']['name']}",
                    "sort_element": f"Количество звезд: {float(response_2['data']['propertySearch']['properties'][count_hotel]['star'])}",
                    "url": f"Ссылка отеля на сайте Hotels: https://www.hotels.com/h{hotel_id}.Hotel-Information"
                }
            )
            count_photo = 3
